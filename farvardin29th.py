...
my_loger.setLevel(logging.INFO)
...
my_loger.addHandler(std_handler)
...
class Book:
    ...
    def read(self, n):
        ...
        try:
            ...
            my_loger.error('ERROR', exc_info=True)
    ...
    def get_info():
        ...
        # try:
        #     with open('file.csv', 'a', newline='') as new_file:
        #         csv_file = csv.writer(new_file)
        #         csv_file.writerow([title, author, publish_year, pages, language, price])
        # except Exception as e:
        #     print('file not found')
        with open('book.csv', 'a', newline='') as new_file:
            fieldnames = ['title', 'author', 'publish_year', 'pages']
        ...
        my_loger.info('a book is added', exc_info=True)
        return media
...
class Magazine(Book):
    ...
    def get_info():
        ...
        with open('file.csv', 'a', newline='') as new_file:
            csv_file = csv.writer(new_file)
            csv_file.writerow([title, author, publish_year, pages, language, price, issue])
        media = Magazine(title, author, publish_year, int(pages), language, price, issue)
        my_loger.info('a magazine is added', exc_info=True)
        return media
...
class PodcastEpisode(Book):
    ...
    def get_info():
        ...
        with open('file.csv', 'a', newline='') as new_file:
            csv_file = csv.writer(new_file)
            csv_file.writerow([title, speaker, publish_year, time, language, price])
        media = PodcastEpisode(title, speaker, publish_year, int(time), language, price)
        my_loger.info('a podcast is added', exc_info=True)
        return media
...
class Audiobook(PodcastEpisode):
    ...
    def get_info():
        ...
        with open('file.csv', 'a', newline='') as new_file:
            csv_file = csv.writer(new_file)
            csv_file.writerow([title, speaker, author, publish_year, pages, time, book_language, audio_language, price])
        media = Audiobook(title, speaker, author, publish_year, int(pages), int(time),
                          book_language, audio_language, price)
        my_loger.info('an audiobook is added', exc_info=True)
        return media
...
def print_media():
    ...
    #           f'{media} \n\33[34mStatus => {media.status}\
    try:
        with open('file.csv', 'r', newline='') as my_file:
